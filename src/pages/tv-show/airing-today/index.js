...
const Popular = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/tv/airing_today?api_key=${API_KEY}&page=${currentPage}`
    )
            ...
            <div className="fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-80 flex items-center justify-center">
              <div className="bg-white p-8 rounded-lg max-w-md w-full">
                <div className="flex justify-between items-center">
                  <h3 className="mt-2 mx-2 text-xl font-bold text-black mb-2">
                    {selectedMovie.title}
                  </h3>
                  <button
                ...
                <div>
                  <div className="flex align-end justify-center gap-10 mt-10">
                    ...
                    <Link
                      href={`/tv-show/${selectedMovie.id}`}
                      className="flex align-end justify-end"
                    >
                      <p className="mt-4 bg-red-600 text-white px-6 py-2 rounded-md hover:bg-red-700">
                        More
                      </p>
                    </Link>
                  </div>
